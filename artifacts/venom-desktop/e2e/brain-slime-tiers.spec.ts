 ...
 * satellite clusters, droplet colony and links that real desktops see are
 ...
 * renderer tier and `slimeScale=` pins the render scale, which makes one
...
/** Keep the pinned scale small: stills need pixels, not resolution. */
const STILL_RENDER_SCALE = 0.35;
...
const readTelemetry = (page: Page) =>
  page.evaluate(
    () => (window as { __venomSlime?: TierTelemetry }).__venomSlime ?? null,
  );

async function openDenseBrain(page: Page, tier: string) {
  await page.goto(
    `/workspace/brain?brainFixture=dense&slimeTier=${tier}&slimeScale=${STILL_RENDER_SCALE}`,
  );
  const region = page.getByRole('region', {
    name: /Knowledge map with 5 nodes/,
  });
...
for (const tier of TIERS) {
  test(`${tier} tier still frame keeps the living layer visible`, async ({
    ...
    const region = await openDenseBrain(page, tier);
    const canvas = region.locator('canvas');
    await expect(canvas).toHaveCount(1);

    // Let the colony populate: droplets need a few simulated steps before
    // their radii grow past zero.
    await page.waitForFunction(
      () =>
        ((window as { __venomSlime?: { frames?: number } }).__venomSlime
          ?.frames ?? 0) >= 8,
      undefined,

 ...
 *
 * The selection is device-local React state persisted only to localStorage —
 * it must never enter the synced workspace blob. Shared-workspace content is
 * served exclusively by membership-checked APIs, and everything cached from
 * them lives in react-query where `evict()` below can remove it the moment
 * the server says access is gone.
 */
...
const storageKeyFor = (userId: string | null) =>
  `venom-shared-space:${userId ?? "anon"}`;
...
export function SharedWorkspaceProvider({ children }: { children: ReactNode }) {
  ...
  }, [listQuery.isSuccess, workspaces, selectedId, selectWorkspace]);
  ...
  // the manual chat fetch). Evict every cached workspace read and return to
  // the personal tier — the personal store is untouched and keeps working.
  useEffect(() => {
    return registerWorkspaceAccessLostHandler(() => {
      ...
      if (selectedIdRef.current) {
        ...
        toast({
          ...
          description:
            "You no longer have access to that workspace. Back in your personal space.",
        });
      }
    });

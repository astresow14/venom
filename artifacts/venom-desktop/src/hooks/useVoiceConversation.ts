 ...
 *
 * Desktop sibling of the mobile hook: auth rides the Clerk session cookie
 * (no bearer tokens), requests use the same relative /api/venom/* paths as
 * the chat page, and mic/output levels are plain mutable refs read by the
 * orb's rAF loop instead of reanimated shared values.
 *
...
} from '@/lib/voice/voiceRestraint';
import {
  createSentenceChunker,
  createSseLineReader,
  type SentenceChunker,
} from '@/lib/voice/voiceSpeech';
import {
...
export type VoiceErrorKind = 'mic' | 'unavailable' | 'unsupported' | 'network';
...
export type VoiceCatalogPreset = {
  ...
  sampleText: string;
  available: boolean;
  /** Short reason shown when a voice can't be used right now. */
...
export type VoiceConversation = {
  ...
  /** Mic level 0..1 — mutated in place; read it from a rAF loop. */
  inputLevelRef: MutableRefObject<number>;
  /** Playback level 0..1 — mutated in place; read it from a rAF loop. */
...
export function useVoiceConversation(
  ...
  const activePresetId: VenomVoicePresetId = normalizeVoicePreferences(
  ...
  const reportDecisionOutcome = useCallback(
    async (decisionId: string, outcome: VoiceDecisionOutcomeKind) => {
      try {
        await fetch('/api/venom/voice/decision-outcome', {
          method: 'POST',
  ...
  finalizeTurnRef.current = (run: VoiceRun, opts: { resume: boolean }) => {
    ...
    if (text.length > 0) {
      addMessage(turn.conversationId, {
        ...
        ...(turn.modelId
          ? {
              modelId: turn.modelId as VenomModelId,
      ...
      // Same background knowledge extraction as typed chat: the server files
      // insights into the ontology store; `filed` carries canonical records.
      if (!turn.skipExtraction) {
        void extractVenomKnowledge({
          ...
          .then((result) => {
            ...
            // Topic classification may have filed some clusters straight
            // into shared workspaces. Their records never merge into the
            // personal blob — refresh the cached workspace knowledge; the
            // notice (with undo) waits on the Brain page rather than
            // interrupting a hands-free conversation.
            ...
            if (result.filedScope?.ownerType === 'org') {
              ...
              // when workspace filings exist — re-filing those locally
              // would duplicate shared content as personal.
              applyKnowledgeInsights(conversationRef, result.clusters);
            }
          })
          .catch((extractionError) => {
            // Voice chat stays usable when extraction is unavailable —
            // except when the server says workspace access is gone, which
            // must evict immediately.
  ...
  const runAssistantTurn = useCallback(
    async (
      ...
      // Same advisory context the chat page sends with a typed message.
      const projectContext = project
        ? `Project: ${project.name}\n${project.description}`
        : undefined;
      const sendingModelId = normalizeModelPreferences(
      ...
      try {
        const response = await fetch('/api/venom/respond', {
          ...
          body: JSON.stringify({
            messages: [
              ...historyMessages.slice(-23).map((message) => ({
                role: message.role,
            ...
            projectId: onScreenProjectId ?? 'proj_default',
            modelId: sendingModelId,
            ...(activeWorkspaceIdRef.current
              ? { workspaceId: activeWorkspaceIdRef.current }
              : {}),
        ...
        if (!response.ok) {
          ...
          try {
            ...
            if (body?.code === WORKSPACE_ACCESS_DENIED_CODE) {
              // Membership ended between selecting the workspace and
              // sending: evict cached workspace content (chat parity).
              notifyWorkspaceAccessLost();
            }
          ...
          throw new VoiceFlowError(
            ...
            response.status === 429
              ...
              : (errorDetail ?? 'The reply could not be generated.'),
          );
        }
        const reader = response.body?.getReader();
        if (!reader) {
  ...
  const requestTurnDecision = useCallback(
    async (
      ...
      const talkativeness: VenomVoiceTalkativeness =
        normalizeVoicePreferences(snapshot.voicePreferences).talkativeness ??
        DEFAULT_VOICE_TALKATIVENESS;
      ...
      try {
        const response = await fetch('/api/venom/voice/decide', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          credentials: 'include',
          body: JSON.stringify({
  ...
  const runOverlappedTurn = useCallback(
    async (
      ...
      const turnPromise = runAssistantTurn(run, text, {
        filing,
      ...
      if (stillHeld) {
        if (!decision || decision.decision === 'respond') {
          ...
          if (decision.decision === 'acknowledge' && decision.acknowledgment) {
            ...
            runAcknowledgeTurn(run, text, decision.acknowledgment, filing);
          } else {
  ...
  const handleUtterance = useCallback(
    async (run: VoiceRun, audioBase64: string) => {
      ...
      try {
        ...
        const decisionPromise = requestTurnDecision(run, text);
        ...
        if (decision === undefined) {
          await runOverlappedTurn(run, text, decisionPromise);
          return;
        }
        ...
        if (!decision || decision.decision === 'respond') {
          ...
          await runAssistantTurn(run, text);
          return;
        }
  ...
  const handleCaptureEvent = useCallback(
    (run: VoiceRun) => (event: VoiceCaptureEvent) => {
      ...
      switch (event.type) {
        case 'level':
          inputLevelRef.current = event.level;
          break;
        case 'speech-start':
    ...
    [disarmWindDown, failSession, handleUtterance],
  );
    ...
    (run: VoiceRun) => (event: VoicePlaybackEvent) => {
      ...
      switch (event.type) {
        case 'started':
          ...
          break;
        case 'level':
          outputLevelRef.current = event.level;
          break;
        case 'finished':
          outputLevelRef.current = 0;
          finalizeTurnRef.current?.(run, { resume: true });
  ...
  const loadCatalog = useCallback(async (): Promise<VoiceCatalogPreset[]> => {
    const response = await fetch('/api/venom/voice/catalog', {
      credentials: 'include',
    });
    if (response.status === 503) {
      throw new VoiceFlowError(
  ...
  const selectPreset = useCallback(
    (id: VenomVoicePresetId) => {
      setVoicePreferences({ presetId: id });
    },
    [setVoicePreferences],
  );

...
import { isWorkspaceAccessDeniedError } from "./workspaceAccess.ts";
...
test("other failures are ordinary errors, not access loss", () => {
  assert.equal(isWorkspaceAccessDeniedError({ status: 403, data: {} }), false);
  assert.equal(
    isWorkspaceAccessDeniedError({ status: 403, data: null }),
    false,
  );
  assert.equal(
    isWorkspaceAccessDeniedError({

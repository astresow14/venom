...
// Stage rules are shared with the desktop app via
// @workspace/venom-workspace-merge so the two normalizers cannot drift: a
// drifted desktop copy used to silently drop duplicate-named columns this app
// still showed, and the board flip-flopped through sync forever. Re-exported
// (from the imported bindings, which the code below also calls) for existing
// call sites and the cross-app identity tests in workspaceMergeRules.test.mjs.
...
function normalizeFields(value: unknown, projectUpdatedAt: number) {
  ...
  return (Array.isArray(value) ? value : [])
    .flatMap((candidate, index): VenomKanbanField[] => {
      ...
      if (!id || !name || seen.has(id) || !FIELD_TYPES.has(type)) return [];
      seen.add(id);

 ...
 * the desktop app at
 * artifacts/venom-desktop/src/components/workspace/SpeakerAvatar.tsx — edit
 ...
 * name as adjacent visible text, so the avatar is hidden from assistive tech
 * (importantForAccessibility maps to aria-hidden on react-native-web).
 */
export function SpeakerAvatar({
  ...
  return (
    <View
      testID={`speaker-avatar-${speakerGlyphKey(glyph)}`}
    ...
    >
      {glyph.kind === "monogram" ? (
        ...
        <VenomMark color={colors.background} size={inner} />
      )}
    </View>
  );
}

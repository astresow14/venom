...
- Transient deliberation state (roster, streaming takes, stage) stays separate from persisted messages; the persisted `deliberation` field comes from the final snapshot, with a client-side fallback built from accumulated takes if the stream dies after voices completed.
